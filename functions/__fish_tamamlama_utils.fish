...
function ukur -d "Sistemde eksik olan araçları crates.io ve Homebrew üzerinden dinamik olarak arayıp kurar"
    ...
    # 1. Cargo (crates.io) üzerinde ara (Her iki sistem için)
    ...
    # 2. İşletim Sistemine Göre Devam Et
    if test "$os_type" = "Darwin"
        # macOS için Homebrew Araması
        ...
        # Linux için
        echo (set_color red)"Hata: '$tool_name' crates.io (Cargo) üzerinde bulunamadı."(set_color normal)
        echo "Linux üzerinde şu an için sadece Rust ekosistemi otomatik kurulabilmektedir."
